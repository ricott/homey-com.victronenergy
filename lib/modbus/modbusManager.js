...
class ModbusManager extends EventEmitter {
    ...
    async #checkAndRestoreConnections() {
        ...
        for (const [clientKey, client] of clientConfigs) {
            ...
            const retryInfo = ModbusManager.retryAttempts.get(clientKey) || { attempts: 0, nextRetryTime: 0 };
            
            if (!this.#isConnected(host, port)) {
                // Skip if we need to wait longer before next retry
                if (currentTime < retryInfo.nextRetryTime) {
                    console.log('DEBUG', `Skipping retry for ${clientKey}, next retry in ${Math.ceil((retryInfo.nextRetryTime - currentTime) / 1000)}s`);
                    continue;
                }

                console.log('WARN', `Connection lost for ${clientKey}, attempt ${retryInfo.attempts + 1}/${this.#maxRetryAttempts}`);
                ...
                try {
                    ...
                    if (retryInfo.attempts >= this.#maxRetryAttempts) {
                        ...
                        const backoffDelay = Math.min(
                            this.#baseRetryDelay * Math.pow(2, retryInfo.attempts),
                            300000 // Cap at 5 minutes
                        );
                        retryInfo.nextRetryTime = currentTime + backoffDelay;
                        ModbusManager.retryAttempts.set(clientKey, retryInfo);
    ...
    async #pollReadings(clientKey, deviceTypeName) {
        ...
        try {
            ...
            if (error.response && error.response._body && error.response._body._code !== undefined) {
                const exceptionMessage = this.#getModbusExceptionMessage(error.response._body._code);
    ...
    #getModbusExceptionMessage(code) {
        const exceptionCodes = {
            ...
            10: 'Gateway Path Unavailable (The gateway was unable to establish a connection to the target device)',

...
class VRM {
    ...
    async getPVInverterForecastRestOfToday(token, siteId) {
        ...
        const response = await this.#makeRequest(token, `https://${config.apiDomain}/v2/installations/${siteId}/stats?` + new URLSearchParams({
            'type': 'custom',
            'attributeCodes[]': 'vrm_pv_inverter_yield_fc',
            'interval': 'days',
            'start': startTs,
            'end': endTs,
        }), 'get');

        return Promise.resolve(response?.totals?.vrm_pv_inverter_yield_fc || undefined);
    }

    async getPVInverterForecastNextDay(token, siteId) {
